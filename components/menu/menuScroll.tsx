			...
			{products.map((item: any, key: any) => {
				return (
					<TouchableOpacity
						...
						style={[selectedMenuItem.Id == item.Id ? styles.selectedButtonWrapper : styles.buttonWrapper, { borderRadius: borderRadius.button, backgroundColor: selectedMenuItem.Id == item.Id ? colors.text : colors.dark_grey, shadowColor: '#ff0e4e' }]}
						onLayout={(event: any) => {
							const layout = event.nativeEvent.layout;
							posArr[key] = layout.x
						}}
					>
						<Text color={selectedMenuItem.Id == item.Id ? colors.primary : colors.background} bold style={[styles.label, { color: selectedMenuItem.Id == item.Id ? colors.primary : colors.background }]}>{item.title}</Text>
					</TouchableOpacity>
